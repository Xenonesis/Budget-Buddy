...
import { ChatInterface, Sidebar, EmptyState } from './components';
...
export default function AIInsightsPage() {
    ...
    try {
      ...
      if (!conversationId) {
        try {
          ...
          console.error(
            'Failed to create conversation, continuing with temporary storage:',
            convError
          );
      ...
      try {
        ...
        response =
          aiResponse ||
          "I'm sorry, I couldn't process your request right now. Please check your AI configuration or try again later.";
      } catch (aiError) {
        console.error('AI Service Error:', aiError);
        response =
          "I'm having trouble connecting to the AI service. Please check your settings or try again in a moment.";
      }
  ...
  const generateConversationTitle = (message: string): string => {
    ...
    const relevantTerm = financialTerms.find((term) => words.some((word) => word.includes(term)));
  ...
  if (!aiEnabled) {
    return (
      <div className="flex items-center justify-center h-screen">
        <EmptyState onConfigureSettings={() => router.push('/dashboard/settings')} />
      </div>
    );
  }
